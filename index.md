# Paybrok Public Documentation

Paybrok is a non-custodial wallet and protected payment platform for people, merchants, freelancers, service providers, and local payment operators who need to use digital dollars in real life.

Paybrok brings together wallet, QR payments, payment links, P2P local liquidity, escrow, service milestones, remittances, providers, disputes, notifications, and security controls in one product experience.

## Start Here

- [English README](README.md)
- [Español](docs/readme-es.md)
- [Features](docs/features.md)
- [Funciones en español](docs/features-es.md)
- [Security](docs/security.md)
- [Seguridad](docs/security-es.md)
- [P2P and Escrow](docs/p2p-escrow.md)
- [P2P y Escrow](docs/p2p-escrow-es.md)
- [Protected Service Payments](docs/service-contracts.md)
- [Pagos Protegidos Por Servicios](docs/service-contracts-es.md)
- [Market Positioning](docs/market-positioning.md)
- [Posicionamiento](docs/market-positioning-es.md)

## Public Website

Official website: [paybrok.com](https://paybrok.com)

Search sitemap: [paybrok.github.io/paybrok/sitemap.xml](https://paybrok.github.io/paybrok/sitemap.xml)
